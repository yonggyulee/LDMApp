...
namespace LDMApp.Modules.DatasetImport.ViewModels
{
    public class DatasetImportDialogViewModel : BindableBase, IDialogAware
    {
        ...
        private async void ExecuteTestCMD()
        {
            string path = "D:\\LocalDatasetManagement\\testdata\\images\\image1_1.jpg";
            using (FileStream stream = File.Open(path, FileMode.Open))
            {
                Image img = new Image
                {
                    ImageID = "EQP1/STEP1/LOT1/WAFER2/DEF1_1.jpg",
                    SampleID = 1,
                    ImageNO = 1,
                    ImageCode = "Right",
                    OriginalFilename = "image1_1.jpg",
                    ImageScheme = "(760,550)",
                    ImageFile = new StreamPart(stream, "image1_1.jpg", "image/jpg")
                };

                await imagesController.UploadImageFileAsync("dataset_5", img.ImageID, img.ImageFile);
            }
        }
        ...
        private void ExecuteInsertDataCMD()
        {
            postSamples();
            RequestClose.Invoke(new DialogResult(ButtonResult.OK));
        }

        private async void postSamples()
        {
            //ExecuteTestCMD();
